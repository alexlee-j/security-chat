# hidden-conversation-sync Specification

## Purpose

定义隐藏会话的跨设备同步机制，包括隐藏/恢复时机、同步协议、数据持久化格式，以及拉黑和消息送达后会话列表可见性的恢复规则。
...
- **AND** 系统 SHALL also set `hidden=true` for 用户 B's conversation with 用户 A
