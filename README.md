...
🚧 **2.0 闭环推进中**：当前以 `security-chat-v2-closed-loop` 作为唯一集成变更，持续收口账户恢复、群治理、同步可靠性与发布门槛
✅ **GPT/MiniMax 分工已明确**：GPT 负责高风险跨层闭环，MiniMax 负责低风险 UI/文档/支撑整合
...
- **账户恢复与认证加固**：注册密码策略、忘记密码/重置密码、重置后的登录态恢复
- **群治理与协议生命周期**：群资料、成员治理、退出/移除/重入与 Sender Key 生命周期一致
- **同步可靠性**：本地消息状态、断线补拉、重登回放、失败重试与摘要一致性
- **发布可运维性**：统一回归矩阵、真实账号冒烟、里程碑级回滚策略与合并门槛

### 下一阶段（独立 change）

- **桌面产品面闭环**：仅在 `security-chat-v2-closed-loop` 合并到 `main` 后，从最新 `main` 新开分支推进
- **范围约束**：不与当前 2.0 核心闭环混分支，不将音视频通话并入当前变更
