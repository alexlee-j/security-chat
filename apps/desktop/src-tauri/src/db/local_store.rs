...
impl SqliteStore {
    ...
    pub fn delete_conversation(&self, id: &str) -> SqliteResult<()> {
        ...
        self.conn.execute("DELETE FROM drafts WHERE conversation_id = ?1", params![id])?;
    ...
    /// 获取会话消息（分页）
    pub fn get_messages(&self, conversation_id: &str, limit: u32, before: Option<i64>) -> SqliteResult<Vec<Message>> {
        match before {
            Some(ts) => {
                let mut stmt = self.conn.prepare(
    ...
    pub fn keychain_retrieve(&self, key_type: &str) -> SqliteResult<Option<Vec<u8>>> {
        let mut stmt = self.conn.prepare(
            "SELECT key_data FROM keychain WHERE key_type = ?1"
        )?;
        ...
        if let Some(row) = rows.next()? {
            ...
            let decoded = base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &data)
                .map_err(|e| rusqlite::Error::InvalidParameterName(format!("base64 decode error: {}", e)))?;
            Ok(Some(decoded))
        } else {
            Ok(None)
        }
    ...
    pub fn keychain_delete(&self, key_type: &str) -> SqliteResult<()> {
        self.conn.execute("DELETE FROM keychain WHERE key_type = ?1", params![key_type])?;
        Ok(())
    }
}
...
mod tests {
    ...
    fn test_conversation_crud() {
        ...
        // Save
        store.save_conversation(&conv).expect("should save conversation");

        // Get
        let result = store.get_conversation("conv1").expect("should get conversation");
        ...
        // Get all
        let all = store.get_conversations().expect("should get all conversations");
        assert_eq!(all.len(), 1);

        // Delete
        store.delete_conversation("conv1").expect("should delete conversation");
        let result = store.get_conversation("conv1").expect("should get conversation");
    ...
    fn test_message_crud() {
        ...
        // Get messages
        let messages = store.get_messages("conv1", 10, None).expect("should get messages");
    ...
    fn test_keychain_crud() {
        ...
        store.keychain_store("key1", "identity", key_data).expect("should store key");

        let retrieved = store.keychain_retrieve("identity").expect("should retrieve key");
        assert!(retrieved.is_some());
        assert_eq!(retrieved.unwrap(), key_data);

        store.keychain_delete("identity").expect("should delete key");
        let retrieved = store.keychain_retrieve("identity").expect("should retrieve key");
    ...
    fn test_unread_count() {
        ...
        let count = store.get_unread_count("conv1").expect("should get unread count");
